...
function PlotDynamicsBiographsWindow(SelectedGeneData,Filtered_CutIntersection_ProbeN,...
         Tcell1DataTable,Tcell2DataTable,GeneNamesCell12, ProbeIDsCell1,...
         PublicIDsCell1,ProbeIDsCell2, PublicIDsCell2,GeneOfInterest,ProbeNumber,...
         PlotBioGraph)
    Cl1_list = [];
    Cl2_list = [];
    ...
    for line = 1:size(Filtered_CutIntersection_ProbeN,1)
        ...
        hFig=figure;
        WindowBegins = 100*ones(round(max([y1(:);y2(:)])),1);
        WindowEnds = 240*ones(round(max([y1(:);y2(:)])),1);      
        WindowLinesHeight = 1:round(max([y1(:);y2(:)]));
        
        subplot(3,1, 1 : 2)
        maxInCellsAllGenes = max(y2(:)); 
        %plot(time_points,y1,'Linewidth',1.5); 
        %xlim([0 360]); 
        plot(time_points,y2,'Linewidth',1.5,'color','r'); xlim([0 360]);
        yMAX = (maxInCellsAllGenes+(maxInCellsAllGenes*(1/4)));
        ylim([0 yMAX]);
        hold on;
        ha = area([0 100], [yMAX yMAX]);
        alpha(.5);
        ha = area([240 360], [yMAX yMAX]);
        alpha(.5);
        hold on;
        plot(WindowBegins,WindowLinesHeight,'black--','Linewidth',0.9);
        plot(WindowEnds,WindowLinesHeight,'black--','Linewidth',0.9);
        ...
        h=legend(['Cell2, ', ProbeIDsCell2CurrentLine, ', ', PublicIDsCell2(line,:),...
...
%         load(FileNameImportantGene)   
        subplot(3,1, 3)
        plot(time_points,SelectedGeneData(2,:),'Linewidth',0.7,'color','r');
        maxInCells = max(SelectedGeneData(:,:)'); 
        maxInProbes = max(maxInCells);
        yMAX = (maxInProbes+maxInProbes*(1/4));
        xlim([0 360]); ylim([0 yMAX])
        hold on;
        ha = area([0 100], [yMAX yMAX]);
        alpha(.5);
        ha = area([240 360], [yMAX yMAX]);
        ...
        plot(WindowBegins,WindowLinesHeight,'black--','Linewidth',0.9);
        plot(WindowEnds,WindowLinesHeight,'black--','Linewidth',0.9);
        MyLegend = legend('Cell-2');
        %set(MyLegend,'color',Green);
        title([num2str(GeneOfInterest) ' ProbeSet ID ' num2str(ProbeNumber) ' in Tregs']);
